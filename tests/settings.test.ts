...

{
  ...
  const s = normalizeSettings({ playerName: 'Ranger', fov: 100, sensitivity: 3.5, volume: 0.25 });
  check('normalizeSettings preserves valid values', s.playerName === 'Ranger' && s.fov === 100 && s.sensitivity === 3.5 && s.volume === 0.25);
}
...
{
  const s = parseSettings(JSON.stringify({ playerName: '\x00\x1f', fov: Number.NaN, sensitivity: 'fast', volume: null }));
