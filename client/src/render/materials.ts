...
/** Deterministic PRNG so the textures look identical every load. */
...
function tex(size: number, seed: number, draw: (ctx: Ctx, s: number, rand: () => number) => void): THREE.CanvasTexture {
...
function speckle(ctx: Ctx, rand: () => number, s: number, count: number, maxSize: number, alpha: number): void {
...
/** Soft translucent stain blotches — wear on the pale deck panels. */
function stains(ctx: Ctx, rand: () => number, s: number, count: number, alpha: number): void {
  for (let i = 0; i < count; i++) {
    const x = rand() * s;
    const y = rand() * s;
    const r = 14 + rand() * 42;
...
function drawBrushedMetal(ctx: Ctx, s: number, rand: () => number, base: [number, number, number]): void {
...
function rivet(ctx: Ctx, x: number, y: number, r: number): void {
